...
import { fetchUserData, toast } from '../App/slice';
    ...
    if (formData) {
      ...
      if (signIn) {
        if (signIn.password === formData.password) {
          ...
          dispatch(toast({ status: "success", message: "Signed in successfully!" }));
          navigate('/');
        } else {
          dispatch(toast({ status: "error", message: "Password Incorrect." }));
        }
      } else {
        dispatch(toast({ status: "error", message: "User does not exist." }));
      }
    }
  };
